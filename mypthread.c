// List all group member's name: Will Keleher, Saketh Mandava, Yuvaraaj Murthy, Harshal Keswani
// username of iLab:
// iLab Server:
...
struct Node* scan_next(struct Queue* this_queue){
    if(this_queue->head != NULL){
...
int mypthread_create(mypthread_t * thread, pthread_attr_t * attr, void*(*function)(void*), void * arg){
    ...
    *thread = count;
    ...
    sched_PSJF(sched->queue, new_node);
    if(init){
...
// change thread state from Running to Ready
    ...
    curr_node->thread_block->status = 0;
    //Enqueue in the scheduler
    sched_PSJF(sched->queue,curr_node);
    curr = dequeue(sched->queue);
    currentThread = curr->thread_block;
    schedule_runner++;
    curr_node->thread_block->time++;
    startTimer();
...
// Deallocated any dynamic memory created when starting this thread
    ...
    if(WaitingThread == NULL){ //Exit is called before join
        ...
        sched_PSJF(sched->queue, WaitingThread);//put in scheduler. So, it can run now.
        currentThread->retVal = value_ptr;
        currentThread->status = -1;
        if(value_ptr!=NULL)
...
// sched_mlfq();
    if(currentThread == NULL){//Runs first time
        ...
        nextThread = scan_next(sched->queue);//peeks at the next thread, does not remove it from queue
...
static void sched_PSJF(struct Queue *this_queue, struct Node* node) {
